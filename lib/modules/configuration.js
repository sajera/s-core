...
class Configuration extends Singleton {
    ...
    environment () {
        ...
        if (pathToEnv) {
            // print info
            this.core.logger.info(highlightLabel('Configuration: environment'), `Trying to extend process.env from ".env" file "${pathToEnv}"`);
            // read
    ...
    initialize ( callback ) {
        ...
        this.core.logger.info(highlightLabel('Configuration: initialize'), `Trying to get config data from "${configPath}".`);
        if (fs.existsSync(configPath+'.json')||fs.existsSync(configPath+'.js')) {
            let configData = require(configPath);
            Object.assign(this, configData);
            this.core.logger.info(successLabel('Configuration: initialize'), 'Configuration was loaded successfully.');
        } else {
            this.core.logger.warn(errorLabel('Configuration: initialize'), 'Failed to load config data.');
        }
        // all done
        callback();
    }
